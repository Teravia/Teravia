// configs/hunian/cluster/validator.ts
//
// Helper untuk memvalidasi field wajib (required) pada section
// spesifikasi cluster. Key error mengikuti konvensi fieldKey yang
// dipakai di Step1Information.tsx: `s{sectionIdx}_f{fieldIdx}`.

import cluster from "./index";

export function validateClusterDetail(
  detailData: Record<string, any>
): Record<string, string> {
  const errors: Record<string, string> = {};

  cluster.forEach((section: any, sectionIdx: number) => {
...
export default validateClusterDetail;
