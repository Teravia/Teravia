// configs/hunian/kontrakan/validator.ts
//
// Helper untuk memvalidasi field wajib (required) pada section
// spesifikasi kontrakan. Key error mengikuti konvensi fieldKey yang
// dipakai di Step1Information.tsx: `s{sectionIdx}_f{fieldIdx}`.

import kontrakan from "./index";

export function validateKontrakanDetail(
  detailData: Record<string, any>
): Record<string, string> {
  const errors: Record<string, string> = {};

  kontrakan.forEach((section: any, sectionIdx: number) => {
...
export default validateKontrakanDetail;
