// configs/hunian/penthouse/validator.ts
//
// Helper untuk memvalidasi field wajib (required) pada section
// spesifikasi penthouse. Key error mengikuti konvensi fieldKey yang
// dipakai di Step1Information.tsx: `s{sectionIdx}_f{fieldIdx}`.

import penthouse from "./index";

export function validatePenthouseDetail(
  detailData: Record<string, any>
): Record<string, string> {
  const errors: Record<string, string> = {};

  penthouse.forEach((section: any, sectionIdx: number) => {
...
export default validatePenthouseDetail;
