// configs/hunian/apartemen/validator.ts
//
// Helper untuk memvalidasi field wajib (required) pada section
// spesifikasi apartemen. Key error mengikuti konvensi fieldKey yang
// dipakai di Step1Information.tsx: `s{sectionIdx}_f{fieldIdx}`.

import apartemen from "./index";

export function validateApartemenDetail(
  detailData: Record<string, any>
): Record<string, string> {
  const errors: Record<string, string> = {};

  apartemen.forEach((section: any, sectionIdx: number) => {
...
export default validateApartemenDetail;
