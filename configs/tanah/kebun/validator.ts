// configs/tanah/kebun/validator.ts
//
// Helper untuk memvalidasi field wajib (required) pada section
// spesifikasi tanah kebun. Key error mengikuti konvensi fieldKey yang
// dipakai di Step1Information.tsx: `s{sectionIdx}_f{fieldIdx}`.

import kebun from "./index";

export function validateKebunDetail(
  detailData: Record<string, any>
): Record<string, string> {
  const errors: Record<string, string> = {};

  kebun.forEach((section: any, sectionIdx: number) => {
...
export default validateKebunDetail;
