// configs/tanah/kavling/validator.ts
//
// Helper untuk memvalidasi field wajib (required) pada section
// spesifikasi tanah kavling. Key error mengikuti konvensi fieldKey yang
// dipakai di Step1Information.tsx: `s{sectionIdx}_f{fieldIdx}`.

import kavling from "./index";

export function validateKavlingDetail(
  detailData: Record<string, any>
): Record<string, string> {
  const errors: Record<string, string> = {};

  kavling.forEach((section: any, sectionIdx: number) => {
...
export default validateKavlingDetail;
