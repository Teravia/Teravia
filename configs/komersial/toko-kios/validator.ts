// configs/komersial/toko-kios/validator.ts
//
// Helper untuk memvalidasi field wajib (required) pada section
// spesifikasi toko/kios. Key error mengikuti konvensi fieldKey yang
// dipakai di Step1Information.tsx: `s{sectionIdx}_f{fieldIdx}`.

import tokoKios from "./index";

export function validateTokoKiosDetail(
  detailData: Record<string, any>
): Record<string, string> {
  const errors: Record<string, string> = {};

  tokoKios.forEach((section: any, sectionIdx: number) => {
...
export default validateTokoKiosDetail;
