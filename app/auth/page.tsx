...
const EyeIcon = ({ show }: { show: boolean }) =>
  show ? (
    ...
    <svg className="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858-5.908a10.046 10.046 0 013.122-.363c4.478 0 8.268 2.943 9.542 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21M3 3l18 18" />
    </svg>
  );
...
export default function AuthPage() {
  ...
  // State User terautentikasi (Poin #3)
  const [currentUser, setCurrentUser] = useState<string | null>(null);

  useEffect(() => {
    // Ambil data user dari localStorage saat pertama dimuat
    const storedUser = localStorage.getItem("teravia_user");
    if (storedUser) {
      setCurrentUser(storedUser);
    }
  }, []);

  // Toggle Visibility Password (Poin #1)
  ...
  // Handle Login & Redireksi (Poin #2 & #3)
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    setTimeout(() => {
      // Ambil username dari email atau simpan nama
      const username = loginEmail.split("@")[0] || "User";
      localStorage.setItem("teravia_user", username);
      setCurrentUser(username);
      setLoading(false);
      
      // Mengarahkan langsung ke Halaman Beranda Utama
      router.push("/");
    }, 1000);
  };

  const handleLogout = () => {
    localStorage.removeItem("teravia_user");
    setCurrentUser(null);
  };
  ...
  const finishRegistration = () => {
    ...
    setTimeout(() => {
      ...
      const registeredName = fullName || "User Baru";
      localStorage.setItem("teravia_user", registeredName);
      ...
      <header className="bg-white border-b border-slate-200 px-6 py-4 flex justify-between items-center">
        ...
        <div>
          {currentUser ? (
            ...
            <span className="text-xs text-slate-400 font-medium">Belum Masuk</span>
          )}
        </div>
      </header>
        ...
        <div className="sm:mx-auto sm:w-full sm:max-w-md">
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full pl-4 pr-10 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-green-500 outline-none text-slate-800"
                    />
                    <button
                      type="button"
          ...
          {mode === "register" && registerStep === 3 && tierData && (
            <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200 shadow-sm">
              ...
              <p className="text-xs text-slate-500 mb-6">
                Langkah 3 dari 3 — Selesaikan pembayaran
              </p>
